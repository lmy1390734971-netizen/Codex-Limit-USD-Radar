...
- 识别任务树中的父任务、子任务和重复累计记录；子代理在自身首个 `turn_context` 前按同会话、直接父任务、任务根的证据顺序继承模型，之后切换为子代理实际模型；仅在祖先—后代链路内去重，保留独立兄弟子代理调用。
- 按每次调用判断 272K 长上下文价格倍率。
- 显示 5 小时和周额度百分比、重置时间及API等价美元额度；美元反推只使用两个同身份额度快照之间的本机API等价成本，并除以这两个快照的实际百分比增量。
...
“已显示本次测量的最新结果；测量仍在继续”表示 Token/成本结果以及可用的5小时、周API等价美元额度已经在同一个完成回调中更新，只是计时尚未结束。界面分别显示“测量时长”和“本次结果计算耗时”，两者不是同一概念。真正计算时会显示后台阶段和已处理记录数，3 秒后提示数据量较大；实时查看 15 秒仍未完成会先取消底层查询并等待工作线程退出，再允许重试，测量和上一次结果不会失效。最终结算保留 60 秒保护，失败时保留冻结结束边界，可再次点击“查看结果”重试；Ready 状态下再次查看也只复用该冻结边界，不会纳入结束后追加的调用。重复的同类查看请求会合并；5 分钟自动刷新和手动“查看结果”都会读取同一冻结点的额度快照，并在存在当前窗口时更新API等价美元额度估算。若最后的子代理Token记录没有附带`rate_limits`，额度估算仍只使用前后两个实际额度快照之间的成本，不会混入快照之后的主时间段成本。
...
- 统计时间、唯一调用数、去重数量；
- 5 小时与周额度百分比、重置时间、API等价美元额度、估算来源及请求级去重完整性；
- 当前内置官方 API 价格表。
...
点击“结束计算”：后台同步变化队列并一次冻结结束 Revision、偏移和同边界额度快照
    ↓
校验窗口、重置时间和账号连续性；以快照区间API等价成本除以同边界实际百分比增量反推美元总额度
```
...
- 5 小时和周窗口独立保存观察时间、窗口长度、重置时间、额度命名空间、credits和可选直接Token容量；Token容量仅作诊断，美元额度始终使用同边界快照成本与百分比增量反推。
...
- 通过任务根 ID、累计 token 和单次 token 组成与模型、事件时间无关的血缘指纹，再结合父子关系仅去除祖先—后代之间的复制事件；父子日志即使以不同时间戳、不同模型归属写入同一调用也只计一次，并采用最浅层祖先记录的模型计价。两个兄弟子代理的相同调用仍分别保留，同一会话中累计量继续前进的真实调用也不会被误删；读取顺序不影响结果。
- `token_records.model_source` 记录模型来自 `turn_context`、同会话、直接父任务、任务根或仍未解析。数据库升级后只扫描确实含空模型记录的源文件执行一次回填；成功后递增索引 Revision 使旧美元缓存失效，失败文件保留为下次重试，不改写 Codex JSONL。
- 新建分支匹配父任务实际存在的事件，剔除测量前继承历史，避免仅按累计量比较造成漏计。
- 真正无法归属或没有官方价格的模型保留“（部分）”标记，不会把未知部分静默按 `$0` 视为完整API成本，也不会使用不完整价格生成新的美元额度估算。
...
指定时间段始终统计该账号本机日志中的全部项目和全部 Codex 会话，不受项目下拉框影响。结束后新写入的日志不会改变结果；开始新的测量会清除上一次的时间段 Token 结果，但同一账号、计划、窗口长度和重置周期内最近一次有效的额度估算会保留，直到本次查询取得新的严格对齐估算。计量期间应保持 Token Rader 窗口运行。
...
只有在计划类型、窗口类型和重置标识一致且没有跨重置时，才会使用严格同边界证据计算：

```text
实际百分比增量 = 结束已用百分比 - 开始已用百分比
快照区间成本 = 冻结日志偏移内 (起始快照时间, 结束快照时间] 的全部本机可观察调用成本
反推总 API 等价美元 = 快照区间成本 / (实际百分比增量 / 100)
预计已用美元 = 反推总额 × 当前已用百分比
预计剩余美元 = 反推总额 × 当前剩余百分比
```

若本次短测量的起止百分比相同，程序不会再用局部成本除以当前累计百分比，也不会凭空把局部成本当作1%。它会在冻结偏移内回查同一计划、窗口和重置周期，寻找最近一个完整的历史百分比步长，再聚合该历史步长两端快照之间的成本。只有整数百分比时通常使用最近的完整1%；日志实际出现0.1%或0.01%变化时直接使用该真实较细步长。没有可靠历史对、百分比下降、跨重置、计划变化或价格不完整时不生成新估算，并仅保留与当前观察快照仍匹配的旧结果。

额度卡按“当前用量 → API等价美元总额度 → 已用/剩余美元 → 起点与校准增量 → 历史回查状态 → 来源与去重完整性”显示。来源`snapshot_delta_usd_estimate`表示美元结果来自快照区间成本和实际百分比增量；`limit_tokens`等Token容量字段不参与美元总额公式。父子代理复制事件先按Token指纹及真实血缘去重，即使时间、模型或turn ID不同也只保留最浅层祖先；没有request/response ID时仍明确标记“请求级去重不完整”。
