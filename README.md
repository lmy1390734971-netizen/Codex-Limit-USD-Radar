...
- 显示 5 小时和周额度百分比、重置时间及API等价美元额度；先估算当前窗口Token总容量，再按完整窗口中实际模型组合的平均API美元/Token换算，避免用短区间价格斜率放大结果。
...
点击“结束计算”：后台同步变化队列并一次冻结结束 Revision、偏移和同边界额度快照
    ↓
校验窗口、重置时间和账号连续性；估算Token容量后以完整窗口平均API美元/Token换算总美元
```
...
- 5 小时和周窗口独立保存观察时间、窗口长度、重置时间、额度命名空间、credits和可选直接Token容量；美元额度使用完整当前重置窗口的Token容量与实际平均API成本换算。
...
有效百分比增量 = 实际增量大于 0 时采用实际值；等于 0 时采用日志百分比最小精度；小于 0 时不反推
反推总 API 等价美元 = 本次 API 等价成本 / (有效百分比增量 / 100)
预计已用美元 = 反推总额 × 当前已用百分比
预计剩余美元 = 反推总额 × 当前剩余百分比
```

额度卡按“当前用量 → API等价美元总额度 → 已用/剩余美元 → 来源与去重完整性”显示。计算先取得当前重置窗口Token容量：若日志携带`limit_tokens`则直接采用，否则使用“窗口本机Token × 100 / 当前已用百分比”。随后以该窗口完整的可观察API成本除以窗口Token，得到实际模型组合的平均美元/Token，并换算总美元。来源分别为`direct_limit_tokens_usd_estimate`和`snapshot_window_usd_estimate`；它们仍是API等价估算，不是套餐账单或官方固定美元池。日志没有request/response ID时会将最近`turn_id`绑定到Token记录以减少跨turn误去重，但一个turn可能包含多次模型调用，因此明确标记“请求级去重不完整”。
