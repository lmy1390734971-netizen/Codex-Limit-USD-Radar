...
有效反推会按“当前用量 → 反推总额度 → 已用/剩余美元 → 从 Y% 开始及精度说明”的顺序显示，其中 Y 是本次测量冻结的起始用量百分比。反推分子不使用主卡片的完整测量成本，而是分别统计5小时、周额度两个快照观察时间之间的全部本机调用，即 `(起始快照, 结束快照]`；冻结偏移保证结算后追加的日志不会进入证据。结束快照早于本次最后一条实际计费调用时不会产生新估算，并继续保留同一账号、计划、窗口和重置周期内最近一次严格对齐的结果。百分比精度、负增量和跨重置规则保持不变。这里的“美元”是按官方 API token 价格换算的等价金额，不是 ChatGPT/Codex 套餐实际账单，也不是官方公布的固定周美元额度。
