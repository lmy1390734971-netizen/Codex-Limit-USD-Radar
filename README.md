...
- 显示 5 小时和周额度百分比、重置时间及Token容量估算；服务端直接提供`limit_tokens`时优先使用，否则按当前重置窗口内的本机Token与已用百分比估算。API等价美元仅作为独立成本展示，不再解释为套餐额度。
...
“已显示本次测量的最新结果；测量仍在继续”表示结果已经完成并显示，只是计时尚未结束。界面分别显示“测量时长”和“本次结果计算耗时”，两者不是同一概念。真正计算时会显示后台阶段和已处理记录数，3 秒后提示数据量较大；实时查看 15 秒仍未完成会先取消底层查询并等待工作线程退出，再允许重试，测量和上一次结果不会失效。最终结算保留 60 秒保护，失败时保留冻结结束边界，可再次点击“查看结果”重试。重复的同类查看请求会合并；5 分钟自动刷新和手动“查看结果”都会读取同一冻结点的额度快照，并在存在当前窗口时更新Token容量估算。
...
- 统计时间、唯一调用数、去重数量；
- 5 小时与周额度百分比、重置时间、Token容量估算、估算来源及请求级去重完整性；
- 当前内置官方 API 价格表。
...
点击“结束计算”：后台同步变化队列并一次冻结结束 Revision、偏移和同边界额度快照
    ↓
校验窗口、重置时间和账号连续性；优先采用服务端直接Token容量，否则以当前重置窗口的本机Token/百分比估算
```
...
- 5 小时和周窗口独立保存观察时间、窗口长度、重置时间、额度命名空间、credits和可选直接Token容量；Token容量估算使用完整当前重置窗口，API美元成本独立计算。
...
- 真正无法归属或没有官方价格的模型保留“（部分）”标记，不会把未知部分静默按 `$0` 视为完整API成本；模型价格完整性不影响独立的Token容量估算。
...
额度卡按“当前用量 → Token总容量 → 已用/剩余Token → 来源与去重完整性”显示。若日志窗口携带`limit_tokens`，来源为`direct_limit_tokens`；当前真实日志未提供该字段时，来源为`snapshot_token_estimate`，公式是“当前重置窗口内本机可观察Token × 100 / 当前已用百分比”。该估算不使用API美元，不把Pro套餐解释为固定美元池。日志没有request/response ID时会将最近`turn_id`绑定到Token记录以减少跨turn误去重，但一个turn可能包含多次模型调用，因此明确标记“请求级去重不完整”。
...
- API 等价美元不等于套餐账单；程序不再把Pro额度换算为固定美元池。Fast/Priority、缓存写入、图片/工具、失败请求和其他客户端仍可能使主成本卡偏少。
- 当前真实JSONL没有服务端`limit_tokens`，Token总容量采用本机窗口Token/百分比估算；其他客户端或不可观察消耗会使该容量偏低。
