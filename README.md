...
- 时间段统计在后台线程计算，日志解析不会卡住界面；文件无变化时重复“查看结果”直接使用缓存结果。
- 内置 C# + SQLite 持久化索引：首次运行导入历史日志，以后每 5 分钟或手动刷新时只打开并解析新增/修改日志；项目和会话列表直接从索引读取。
...
首次启动会在项目的 `data/private/index/` 中建立本地索引，因此历史日志较多时第一次可能稍慢；后续刷新只读取新增/修改内容。界面默认显示最近 30 天，可在顶部切换历史范围。
...
发现 JSONL 日志
    ↓
读取全量文件元数据，并使用文件级增量缓存
    ↓
点击“开始计算”：冻结所有文件的起始字节偏移和 5 小时/周额度起始快照
    ↓
...
按模型价格和长上下文倍率逐调用计价
    ↓
点击“结束计算”：只读取各文件截至该次点击的结束偏移，并在相同边界选取额度快照
    ↓
...
- 所有日志文件首次写入项目内的持久化索引；后续同步只重读新增或发生变化的文件，不以固定的最近文件数量截断搜索。常规同步会轻量比较文件长度和修改时间，但不会重复打开未变化日志的内容。
...
顶部“历史显示范围”只过滤会话列表与项目累计的候选日志，默认最近 30 天。“立即刷新结果”会先同步新增/修改日志；“清理30天前索引”会删除旧 SQLite 记录并记住清理界限，后续刷新不会马上重新导入这些旧日志；“完整重建”用于恢复全部历史或处理索引损坏、日志结构升级。开始/结束指定时间段始终覆盖起止边界内的全部原始日志，不受历史范围、索引清理和项目选择影响。

被清理的旧会话如果之后重新打开并写入新内容，文件修改时间会更新，因此会重新进入索引。清理和完整重建均只操作 `data/private/index/index.db`，不会删除或改写 `~/.codex/sessions`。

派生/子任务的累计 token 可能包含父任务历史。为避免重复，程序保留 token 原值显示，但不会直接把派生任务的累计值换算成美元。

### 指定时间段

1. 工作开始前点击“开始计算”；程序记录全部日志文件的起始偏移，并冻结当时可用的 5 小时/周额度快照；
2. 正常使用 Codex；
3. 点击“立即刷新结果”或“查看结果”可查看截至当前时刻的临时增量，但不会结束测量；
4. 工作完成后点击“结束计算”；程序只读取结束按钮点击时各文件的长度，并将该边界内的 token、成本和额度快照一起冻结。
...
- 索引随程序关闭保留，以便下次只同步新增/修改日志；`data/private/` 已由 `.gitignore` 排除，不会被正常 Git 提交。“清理30天前索引”删除旧缓存记录，“完整重建”删除并重建整个可恢复索引，两者都不接触原始日志。
