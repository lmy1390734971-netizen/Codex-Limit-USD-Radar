...
```text
有效百分比增量 = 结束已用百分比 - 开始已用百分比（必须大于 0）
反推总 API 等价美元 = 本次 API 等价成本 / (有效百分比增量 / 100)
预计已用美元 = 反推总额 × 当前已用百分比
预计剩余美元 = 反推总额 × 当前剩余百分比
```

有效反推会直接显示“美金额度≈$X · 从 Y% 开始 · 已用≈$A · 剩余≈$B”，其中 Y 是本次测量冻结的起始用量百分比。计算直接采用日志提供的最小百分比精度，不设置人为下限：增量为 `0.1%` 就按 `0.1%`，日志若提供 `0.01%` 就按 `0.01%`；增量为 0 或下降时仍不反推。测量进行中的5分钟自动预览和手动“查看结果”都会同步额度快照，只要已经产生可计价消耗和正增量就直接显示总额度，无需等到结束计算。这里的“美元”是按官方 API token 价格换算的等价金额，不是 ChatGPT/Codex 套餐实际账单，也不是官方公布的固定周美元额度。Pro 5x 的百分比已经相对于 5x 计划额度计算，不能再把结果乘以 5。窗口重置、计划/窗口/重置标识不一致、未知模型价格或起止边界不完整时显示“尚无有效反推结果”，并清除该窗口的旧估算。
