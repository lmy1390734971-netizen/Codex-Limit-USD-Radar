...
“已显示本次测量的最新结果；测量仍在继续”表示结果已经完成并显示，只是计时尚未结束。界面分别显示“测量时长”和“本次结果计算耗时”，两者不是同一概念。真正计算时会显示后台阶段和已处理记录数，3 秒后提示数据量较大；实时查看 15 秒仍未完成会先取消底层查询并等待工作线程退出，再允许重试，测量和上一次结果不会失效。最终结算保留 60 秒保护，失败时保留冻结结束边界，可再次点击“查看结果”重试。重复的同类查看请求会合并；5 分钟自动刷新和手动“查看结果”都会读取同一冻结点的额度快照，因此已有可计价成本后即可在测量进行中直接反推总额度。
...
- 通过任务根 ID、规范化模型、累计 token 和单次 token 组成血缘指纹，再结合父子关系仅去除祖先—后代之间的复制事件；父子日志即使以不同时间戳写入同一调用也只计一次，两个兄弟子代理的相同调用仍分别保留，同一会话中累计量继续前进的真实调用也不会被误删。父任务关系晚于孙任务到达时，会事务性回填文件游标和 token 记录的规范任务根。
...
- 真正无法归属或没有官方价格的模型保留“（部分）”标记，不会把未知部分静默按 `$0` 视为完整成本，也不会用部分成本生成新的额度反推。
...
有效反推会按“当前用量 → 反推总额度 → 已用/剩余美元 → 从 Y% 开始及精度说明”的顺序显示，其中 Y 是本次测量冻结的起始用量百分比。计算直接采用日志提供的最小百分比精度，不设置固定人为下限：真实增量为 `0.1%` 就按 `0.1%`，日志若提供 `0.01%` 就按 `0.01%`。已有可计价成本但百分比尚未出现可见变化时，先按日志可表达的最小步长给出直接估算并明确标注；一旦出现真实正增量，立即改用真实增量重算。点击“查看结果”若暂时缺少新额度快照、价格暂时不完整或没有形成新的反推基准，会继续显示同一账号、计划、窗口和重置周期内最近一次有效结果；账号、计划、窗口或重置周期真正变化时才清除。已经到达重置时间的窗口显示“暂无当前窗口”，不得用于反推。百分比下降仍不反推。测量进行中的5分钟自动预览和手动“查看结果”都会同步额度快照，因此无需等到结束计算。这里的“美元”是按官方 API token 价格换算的等价金额，不是 ChatGPT/Codex 套餐实际账单，也不是官方公布的固定周美元额度。Pro 5x 的百分比已经相对于 5x 计划额度计算，不能再把结果乘以 5。
